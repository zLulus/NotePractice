...
namespace CommodityPrice.Train
{
    ...
    /// </summary>
    public class CommodityPriceMLModel
    {
        //数据所在位置
        static string _dataPath = Path.Combine(Environment.CurrentDirectory, "TrainData", "crypto-markets-modify.csv");
        //assign the Number of records in dataset file to constant variable
        //共36条数据
        static int _docsize = 2042;
        public static void Train()
        {
            //引用Microsoft.ML
            MLContext mlContext = new MLContext();
            //加载数据
            IDataView dataView = mlContext.Data.LoadFromTextFile<CommodityPriceModel>(path: _dataPath, hasHeader: true, separatorChar: ',');
        ...
        private static void DetectIidSpike(MLContext mlContext, IDataView dataView)
        {
            ...
            var predictions = mlContext.Data.CreateEnumerable<CommodityPricePrediction>(transformedData, reuseRowObject: false);
        ...
        private static void DetectIidChangePoint(MLContext mlContext, IDataView dataView)
        {
            ...
            foreach (var p in predictions)
            {
                ...
                if (p.Prediction[0] == 1)
                {
                    results += " <-- alert is on, predicted changepoint  警报开启，预测变化点";
                }
                Console.WriteLine(results);
            }
            Console.WriteLine("");
        ...
        static IDataView CreateEmptyDataView(MLContext mlContext)
        {
            ...
            IEnumerable<CommodityPriceModel> enumerableData = new List<CommodityPriceModel>();
